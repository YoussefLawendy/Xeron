...
export default function JobDetailPage({ params }) {
    ...
    return (
        <div className="pt-32 pb-20 px-6">
            ...
            text-content-primary tracking-tight mb-4">
                        {job.title}
                    </h1>

                    <p className="text-[16px] text-content-secondary leading-[1.8]">
                        {job.description}
                    </p>
                </div>
                ...
                {/* Requirements */}
                <div className="mb-10">
                    <h2 className="font-display text-xl font-bold text-content-primary mb-5">
                        Requirements
                    </h2>
                    <ul className="flex flex-col gap-3">
                        {job.requirements.map((req) => (
                            <li key={req} className="flex items-start gap-3 text-[14px] text-content-secondary">
                ...
                {job.nice?.length > 0 && (
                    <div className="mb-12">
                        ...
                        <ul className="flex flex-col gap-3">
                            {job.nice.map((n) => (
                                <li key={n} className="flex items-start gap-3 text-[14px] text-content-secondary">
          ...
          sm:flex-row items-start sm:items-center justify-between gap-6">
                    <div>
                        <h3 className="font-display font-bold text-content-primary mb-1">
                            Interested?
                        </h3>
